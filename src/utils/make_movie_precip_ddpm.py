...
precip_mp4 = os.path.join(output_dir, "precipitation_movie.mp4")
...
for file_name in tqdm(sample_files[:num_samples_to_be_used], desc="Generating Movie Frames"):
    ...
    with np.load(file_path) as data:
        ...
        for sample_idx in range(num_samples):  # Loop over all samples
            # ========================== Precipitation ==========================
            hres_tp6hr = data['hres'][sample_idx][1] * precip_std_hr + precip_mean_hr
            output_tp6hr = data['output'][sample_idx][1] * precip_std_hr + precip_mean_hr
            lres_interp_tp6hr = data['lres_interp'][sample_idx][5] * precip_std_hr + precip_mean_hr
            # lucie_zero_shot_tp6hr = data['lucie_zero_shot'][sample_idx][1] * precip_std_hr + precip_mean_hr

            # ======================== Create Figures ========================
            # ======================== Precipitation Plot ========================
            fig_precip, axs_precip = plt.subplots(1, 4, figsize=(40, 12), subplot_kw={'projection': projection})
            precip_data = [
                (lres_interp_tp6hr * 1000, "ERA5 Interpolated", loncoords_hres, latcoords_hres),
                (hres_tp6hr * 1000, "ERA5 (hres)", loncoords_hres, latcoords_hres),
                (output_tp6hr * 1000, "ERA5 Diffusion", loncoords_hres, latcoords_hres),
                # (lucie_zero_shot_tp6hr * 1000, "LUCIE Zero-Shot", loncoords_hres, latcoords_hres)
            ]
            ...
            for ax, (data_points, title, loncoords, latcoords) in zip(axs_precip, precip_data):
                ...
                #     vmin=0, vmax=15, transform=projection, cmap=ocean.cm.rain, origin="upper"
                # )
                im = ax.pcolormesh(
                    loncoords, latcoords, data_points,
                    vmin=0, vmax=15, transform=projection, cmap=ocean.cm.rain, shading='auto',
                )
                ax.coastlines()

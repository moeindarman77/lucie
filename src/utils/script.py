...
for key in means.keys():
    if key == 'output' or key == 'hres' or key == 'lucie_zero_shot':
        ...
        means[key][4] = means[key][4] * precip_std_hr + precip_mean_hr
...
##################################### Plot temperature ####################################
fig, axs = plt.subplots(4, 1, figsize=(10, 25), subplot_kw={'projection': projection})
data_to_plot = [
    (means['lres'][0], "LR ERA5", loncoords_lres, latcoords_lres),
    (means['hres'][0], "HR ERA5", loncoords_hres, latcoords_hres),
    (means['output'][0], "Model Output", loncoords_hres, latcoords_hres),
...
fig, axs = plt.subplots(4, 1, figsize=(10, 25), subplot_kw={'projection': projection})
data_to_plot = [
    (means['lres'][1]*1000, "LR ERA5", loncoords_lres, latcoords_lres),
    (means['hres'][1]*1000, "HR ERA5", loncoords_hres, latcoords_hres),
    (means['output'][1]*1000, "Model Output", loncoords_hres, latcoords_hres),
    # (means['lucie_zero_shot'][1], 'LUCIE Zero Shot', loncoords_hres, latcoords_hres)
]
ims = []
for ax, (data, title, loncoords, latcoords) in zip(axs, data_to_plot):
    im = ax.pcolormesh(loncoords, latcoords, data, vmin=0, vmax=20, transform=projection, cmap=ocean.cm.haline)
    ax.coastlines()
    ax.set_title(f"{title} - Specific Humidity Climatology", fontsize=12, fontweight="bold")
    ims.append(im)
cbar = fig.colorbar(ims[0], ax=axs, orientation="horizontal", fraction=0.046, pad=0.04)
cbar.set_label("Specific Humidity (g/kg)")
plt.savefig(os.path.join(output_dir, "specific_humidity_climatology.png"), dpi=1200, bbox_inches="tight", format="png")
logging.info("Saved specific humidity comparison plot.")
...
data_to_plot = [
    ...
    (means['output'][2], "Model Output", loncoords_hres, latcoords_hres),
    # (means['lucie_zero_shot'][2], 'LUCIE Zero Shot', loncoords_hres, latcoords_hres)
]
ims = []
for ax, (data, title, loncoords, latcoords) in zip(axs, data_to_plot):
    im = ax.pcolormesh(loncoords, latcoords, data, vmin=-40, vmax=40, transform=projection, cmap=ocean.cm.speed)
    ax.coastlines()
...
data_to_plot = [
    ...
    (means['hres'][3], "HR ERA5", loncoords_hres, latcoords_hres),
    (means['output'][3], "Model Output", loncoords_hres, latcoords_hres),
    # (means['lucie_zero_shot'][3], 'LUCIE Zero Shot', loncoords_hres, latcoords_hres)
]
ims = []
for ax, (data, title, loncoords, latcoords) in zip(axs, data_to_plot):
    im = ax.pcolormesh(loncoords, latcoords, data, vmin=-20, vmax=20, transform=projection, cmap=ocean.cm.speed)
    ax.coastlines()
...
data_to_plot = [
    ...
    (means['hres'][4]*1000, "HR ERA5", loncoords_hres, latcoords_hres),
    (means['output'][4]*1000, "Model Output", loncoords_hres, latcoords_hres),
...
fig, axs = plt.subplots(4, 1, figsize=(10, 25), subplot_kw={'projection': projection})
data_to_plot = [
    (means['lres'][5], "LR ERA5", loncoords_lres, latcoords_lres),
    (means['hres'][5], "HR ERA5", loncoords_hres, latcoords_hres),
    (means['output'][5], "Model Output", loncoords_hres, latcoords_hres),
    # (means['lucie_zero_shot'][5], 'LUCIE Zero Shot', loncoords_hres, latcoords_hres)
]
ims = []
for ax, (data, title, loncoords, latcoords) in zip(axs, data_to_plot):
    im = ax.pcolormesh(loncoords, latcoords, data, transform=projection, cmap=ocean.cm.tempo)
    ax.coastlines()
    ax.set_title(f"{title} - Geopotential Climatology", fontsize=12, fontweight="bold")
    ims.append(im)
cbar = fig.colorbar(ims[0], ax=axs, orientation="horizontal", fraction=0.046, pad=0.04)
cbar.set_label("Geopotential (m)")
plt.savefig(os.path.join(output_dir, "geopotential_climatology.png"), dpi=1200, bbox_inches="tight", format="png")
logging.info("Saved geopotential comparison plot.")
...
hres_shum_mean, hres_shum_std = compute_mean_std_spectrum_torch(torch.tensor(means['hres'][1]).unsqueeze(0))
output_shum_mean, output_shum_std = compute_mean_std_spectrum_torch(torch.tensor(means['output'][1]).unsqueeze(0))
lres_shum_mean, lres_shum_std = compute_mean_std_spectrum_torch(torch.tensor(means['lres'][1]).unsqueeze(0))
lres_interp_shum_mean, lres_interp_shum_std = compute_mean_std_spectrum_torch(torch.tensor(means['lres_interp'][1]).unsqueeze(0))
# lucie_shum_mean, lucie_shum_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie'][1]).unsqueeze(0))
# lucie_interp_shum_mean, lucie_interp_shum_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie_interp'][1]).unsqueeze(0))
# lucie_zero_shot_shum_mean, lucie_zero_shot_shum_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie_zero_shot'][1]).unsqueeze(0))

hres_u_mean, hres_u_std = compute_mean_std_spectrum_torch(torch.tensor(means['hres'][2]).unsqueeze(0))
output_u_mean, output_u_std = compute_mean_std_spectrum_torch(torch.tensor(means['output'][2]).unsqueeze(0))
...
# lucie_u_mean, lucie_u_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie'][2]).unsqueeze(0))
# lucie_interp_u_mean, lucie_interp_u_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie_interp'][2]).unsqueeze(0))
# lucie_zero_shot_u_mean, lucie_zero_shot_u_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie_zero_shot'][2]).unsqueeze(0))

hres_v_mean, hres_v_std = compute_mean_std_spectrum_torch(torch.tensor(means['hres'][3]).unsqueeze(0))
output_v_mean, output_v_std = compute_mean_std_spectrum_torch(torch.tensor(means['output'][3]).unsqueeze(0))
...
# lucie_v_mean, lucie_v_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie'][3]).unsqueeze(0))
# lucie_interp_v_mean, lucie_interp_v_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie_interp'][3]).unsqueeze(0))
# lucie_zero_shot_v_mean, lucie_zero_shot_v_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie_zero_shot'][3]).unsqueeze(0))

hres_precip_mean, hres_precip_std = compute_mean_std_spectrum_torch(torch.tensor(means['hres'][4]).unsqueeze(0))
output_precip_mean, output_precip_std = compute_mean_std_spectrum_torch(torch.tensor(means['output'][4]).unsqueeze(0))
...
# lucie_precip_mean, lucie_precip_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie'][1]).unsqueeze(0))
# lucie_interp_precip_mean, lucie_interp_precip_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie_interp'][1]).unsqueeze(0))
# lucie_zero_shot_precip_mean, lucie_zero_shot_precip_std = compute_mean_std_spectrum_torch(torch.tensor(means['lucie_zero_shot'][1]).unsqueeze(0))
...
################################################################ Specific Humidity Spectrum ################################################################
plt.figure(figsize=(10, 6))

# Plot High-Resolution ERA5 Spectrum (Specific Humidity)
plt.loglog(wavenumbers, hres_shum_mean, label="ERA5 (hres)", color="black", linewidth=2)
plt.fill_between(wavenumbers, hres_shum_mean - hres_shum_std, hres_shum_mean + hres_shum_std, color="black", alpha=0.3)

# Plot VAE Output Spectrum (Specific Humidity)
plt.loglog(wavenumbers, output_shum_mean, label="ERA5 DS", color="red", linewidth=2)
plt.fill_between(wavenumbers, output_shum_mean - output_shum_std, output_shum_mean + output_shum_std, color="red", alpha=0.3)

plt.loglog(wavenumbers, lres_interp_shum_mean, label="ERA5 interpolated", color="green", linewidth=2)
plt.fill_between(wavenumbers, lres_interp_shum_mean - lres_interp_shum_std, lres_interp_shum_mean + lres_interp_shum_std, color="green", alpha=0.3)

# plt.loglog(wavenumbers, lucie_interp_shum_mean, label="LUCIE interpolated", color="purple", linewidth=2)
# plt.fill_between(wavenumbers, lucie_interp_shum_mean - lucie_interp_shum_std, lucie_interp_shum_mean + lucie_interp_shum_std, color="purple", alpha=0.3)

# plt.loglog(wavenumbers, lucie_zero_shot_shum_mean, label="LUCIE Zero Shot", color="blue", linewidth=2)
# plt.fill_between(wavenumbers, lucie_zero_shot_shum_mean - lucie_zero_shot_shum_std, lucie_zero_shot_shum_mean + lucie_zero_shot_shum_std, color="blue", alpha=0.3)

plt.title("Specific Humidity Spectrum")
plt.xlabel("Wavenumber")
plt.ylabel("Mean Magnitude")
plt.legend()
plt.grid(True, which="both", ls="--", lw=0.5)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "spectrum_comparison_shum.pdf"))#, dpi=300)
logging.info("Saved temperature and specific humidity spectrum comparison plots.")
################################################################ U-Wind Spectrum ################################################################
plt.figure(figsize=(10, 6))

# Plot High-Resolution ERA5 Spectrum (U-Wind)
...
# Plot VAE Output Spectrum (U-Wind)
plt.loglog(wavenumbers, output_u_mean, label="ERA5 DS", color="red", linewidth=2)
plt.fill_between(wavenumbers, output_u_mean - output_u_std, output_u_mean + output_u_std, color="red", alpha=0.3)

plt.loglog(wavenumbers, lres_interp_u_mean, label="ERA5 interpolated", color="green", linewidth=2)
...
# plt.loglog(wavenumbers, lucie_interp_u_mean, label="LUCIE interpolated", color="purple", linewidth=2)
# plt.fill_between(wavenumbers, lucie_interp_u_mean - lucie_interp_u_std, lucie_interp_u_mean + lucie_interp_u_std, color="purple", alpha=0.3)

# plt.loglog(wavenumbers, lucie_zero_shot_u_mean, label="LUCIE Zero Shot", color="blue", linewidth=2)
# plt.fill_between(wavenumbers, lucie_zero_shot_u_mean - lucie_zero_shot_u_std, lucie_zero_shot_u_mean + lucie_zero_shot_u_std, color="blue", alpha=0.3)

plt.title("U-Wind Spectrum")
plt.xlabel("Wavenumber")
plt.ylabel("Mean Magnitude")
plt.legend()
plt.grid(True, which="both", ls="--", lw=0.5)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "spectrum_comparison_u_wind.pdf"))#, dpi=300)
logging.info("Saved temperature and u-wind spectrum comparison plots.")
################################################################ V-Wind Spectrum ################################################################
plt.figure(figsize=(10, 6))

# Plot High-Resolution ERA5 Spectrum (V-Wind)
...
# Plot VAE Output Spectrum (V-Wind)
plt.loglog(wavenumbers, output_v_mean, label="ERA5 DS", color="red", linewidth=2)
plt.fill_between(wavenumbers, output_v_mean - output_v_std, output_v_mean + output_v_std, color="red", alpha=0.3)

plt.loglog(wavenumbers, lres_interp_v_mean, label="ERA5 interpolated", color="green", linewidth=2)
...
# plt.loglog(wavenumbers, lucie_interp_v_mean, label="LUCIE interpolated", color="purple", linewidth=2)
# plt.fill_between(wavenumbers, lucie_interp_v_mean - lucie_interp_v_std, lucie_interp_v_mean + lucie_interp_v_std, color="purple", alpha=0.3)

# plt.loglog(wavenumbers, lucie_zero_shot_v_mean, label="LUCIE Zero Shot", color="blue", linewidth=2)
# plt.fill_between(wavenumbers, lucie_zero_shot_v_mean - lucie_zero_shot_v_std, lucie_zero_shot_v_mean + lucie_zero_shot_v_std, color="blue", alpha=0.3)

plt.title("V-Wind Spectrum")
plt.xlabel("Wavenumber")
plt.ylabel("Mean Magnitude")
plt.legend()
plt.grid(True, which="both", ls="--", lw=0.5)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "spectrum_comparison_v_wind.pdf"))#, dpi=300)
...
plt.grid(True, which="both", ls="--", lw=0.5)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "spectrum_comparison_percip.pdf"))#, dpi=300)
...
metrics_output_vs_hres = {
    "Temperature": {}, "SpecificHumidity": {}, "U-Wind": {}, "V-Wind": {}, "Precipitation": {}
}
metrics_lres_interp_vs_hres = {
    "Temperature": {}, "SpecificHumidity": {}, "U-Wind": {}, "V-Wind": {}, "Precipitation": {}
}

# Mapping from title to dictionary key
title_to_key = {
...
# Iterate over both channels (Temperature = 0, SpecificHumidity = 1, etc.)
for channel, metric_name in enumerate(["Temperature", "SpecificHumidity", "U-Wind", "V-Wind", "Precipitation"]):
...
def format_metrics_table(metrics_dict, title):
    ...
    category_key = title_to_key[title]  # Use mapping instead of split()
    
    table = f"\n{'='*40}\n{title}\n{'='*40}\n"
    table += "{:<25} {:<15} {:<15}\n".format("Metric", "Model", "Interpolation")
    table += "-"*75 + "\n"
    
    for metric in metrics_dict.keys():  # Get metric names dynamically
        table += "{:<25} {:<15.4f} {:<15.4f}\n".format(
            metric,
            metrics_output_vs_hres[category_key][metric],  
            metrics_lres_interp_vs_hres[category_key][metric]
        )
    return table

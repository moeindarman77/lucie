...
# Example Usage: 
# python -u make_movie_temperature.py --task_name vae_concat_LUCIE_v5 --num_samples 200
...
temp_gif = os.path.join(output_dir, "temperature_movie.gif")
precip_gif = os.path.join(output_dir, "precipitation_movie.gif")
...
temp_mp4 = os.path.join(output_dir, "temperature_movie.mp4")
...
for file_name in tqdm(sample_files[:num_samples_to_be_used], desc="Generating Movie Frames"):
    ...
    with np.load(file_path) as data:
        ...
        for sample_idx in range(num_samples):  # Loop over all samples
            # ========================== Temperature ==========================
            hres_temp = data['hres'][sample_idx][0] * temperature_std_hr + temperature_mean_hr
            output_temp = data['output'][sample_idx][0] * temperature_std_hr + temperature_mean_hr
            lres_interp_temp = data['lres_interp'][sample_idx][0] * temperature_std_hr + temperature_mean_hr
            # lucie_zero_shot_temp = data['lucie_zero_shot'][sample_idx][0] * temperature_std_hr + temperature_mean_hr

            # ======================== Create Figures ========================
            # Temperature Plot
            # fig_temp, axs_temp = plt.subplots(1, 4, figsize=(40, 12), subplot_kw={'projection': projection})
            # fig_temp, axs_temp = plt.subplots(1, 4, figsize=(20, 6), subplot_kw={'projection': projection})
            fig_temp, axs_temp = plt.subplots(1, 4, figsize=(40, 11.25), subplot_kw={'projection': projection})
            temp_data = [
                (lres_interp_temp, "ERA5 Interpolated", loncoords_hres, latcoords_hres),
                (hres_temp, "ERA5 (hres)", loncoords_hres, latcoords_hres),
                (output_temp, "ERA5 Diffusion model", loncoords_hres, latcoords_hres),
                # (lucie_zero_shot_temp, "Lucie Zero-Shot", loncoords_hres, latcoords_hres)
            ]
            ...
            for ax, (data_points, title, loncoords, latcoords) in zip(axs_temp, temp_data):
                ...
                # im = ax.imshow(
                #     data_points, extent=[loncoords.min(), loncoords.max(), latcoords.min(), latcoords.max()],
                #     vmin=200, vmax=300, transform=projection, cmap=ocean.cm.thermal, origin="upper"
                # )
                ax.coastlines()

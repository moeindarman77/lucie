...
def sample(model, scheduler, train_config, diffusion_model_config,
    ...
    xt = torch.randn((1,
                      autoencoder_model_config['z_channels']*2,
                      91,
                      180)).to(device)
    ...
    for i in tqdm(reversed(range(diffusion_config['num_timesteps']))):
        ...
        if i == 0:
            ...
            mean, logvar = torch.chunk(xt, 2, dim=1)
            std = torch.exp(0.5 * logvar)
            # sample = mean + std * torch.randn(mean.shape).to(device=xt.device)
            sample = mean
            ims = vae.decode(sample, encoded_features)
            ims = ims[:,:,3:3+721,:]
...
def infer(args):
    ...
    model = FNO2d(
        input_channels=dataset_config['input_channels']+autoencoder_model_config['z_channels']*2, 
         output_channels=8,
    ...
    if os.path.exists(os.path.join(task_dir,
        ...
        checkpoint = torch.load("/glade/derecho/scratch/mdarman/lucie/results/vae_concat_v6/checkpoints/best_ldm_current.pth")
        new_state_dict = {}

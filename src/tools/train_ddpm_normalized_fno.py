...
def train(rank, world_size, args):
    ...
    logging.info(f"Loading FNO normalization statistics from {fno_stats_path}")
    ...
    logging.info(f"FNO normalization stats loaded (scalar). Values: mean={fno_mean.squeeze()}, std={fno_std.squeeze()}")
    ...
    # scheduler_optimizer = ReduceLROnPlateau(optimizer, 'min', factor=0.1, patience=5, min_lr=1e-6)  # Commented out - keeping LR constant at 1e-5

    criterion = torch.nn.MSELoss()

    # Define the path to your checkpoint file
    # Priority: 1) Latest checkpoint in new task dir, 2) Pretrained checkpoint, 3) Start from scratch

    # Check for the latest checkpoint in the current task directory
    latest_checkpoint = os.path.join(checkpoint_dir, train_config['ldm_ckpt_name'])  # ddpm_ckpt.pth
    pretrained_checkpoint = '/glade/derecho/scratch/mdarman/lucie/results/unet_final_v10_retrain/checkpoints/ddpm_ckpt.pth'

    start_epoch = 0  # Default starting epoch
    resume_checkpoint = None

    # Check for latest checkpoint (jobs have already started, so this should exist)
    if os.path.exists(latest_checkpoint):
        resume_checkpoint = latest_checkpoint
        logging.info(f"Found latest checkpoint in task directory: {latest_checkpoint}")
    # Fall back to pretrained checkpoint if no checkpoint exists in current task dir
    elif os.path.exists(pretrained_checkpoint):
        resume_checkpoint = pretrained_checkpoint
        logging.info(f"Using pretrained checkpoint: {pretrained_checkpoint}")
    else:
        logging.info(f"No checkpoint found at {latest_checkpoint} or {pretrained_checkpoint}, starting from scratch.")
    ...
    for epoch_idx in range(start_epoch, num_epochs):
        ...
        for batch_idx, data in enumerate(tqdm(data_loader, disable=(rank != 0))):
            ...
            # fno_output shape: (batch, 4, 721, 1440)
            ...
            cond_input = torch.cat([fno_output_normalized, hres[:, 5:6]], dim=1) # Concatenate the normalized FNO output with the orography as conditioning
        ...
        # current_lr = scheduler_optimizer.get_last_lr()[0]  # Commented out - using fixed LR
        current_lr = train_config['ldm_lr']  # Fixed learning rate

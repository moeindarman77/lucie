...
from models.fno_ddpm import FNO2d
from models.vqvae import VQVAE
from models.vae_hr import VAE
...
def train(rank, world_size, args):
    ...
    if rank==0 and not os.path.exists(task_dir):
        os.mkdir(task_dir, exist_ok=True)
    ...
    model = FNO2d(
        input_channels=8+autoencoder_config['z_channels'], 
         output_channels=4,
    ...
    if not train_config['load_latents']:
        ...
        print("Datset input: ", dataset_config['input_channels'])
        ...
        if os.path.exists(vae_load_dir):
            ...
            #                                map_location=device,
            #                                weights_only=True)['model_state_dict'])
            checkpoint = torch.load("/glade/derecho/scratch/mdarman/lucie/results/vae_hr/checkpoints/vae_autoencoder_ckpt_epoch_15.pth", 
                                    )
    ...
    scheduler_optimizer = ReduceLROnPlateau(optimizer, 'min', factor=0.1, patience=5, min_lr=1e-6, verbose=True)
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_idx, data in enumerate(tqdm(data_loader, disable=(rank != 0))):
            ...
            with torch.no_grad():
                # Settihg seed for reproducibility
                torch.manual_seed(42)
                latent_sample, latent_distribution, _ = vae.module.encode(hres_padded)
            ...
            noise = torch.randn_like(latent_sample).to(rank)
            
            # Sample timesptep
            t = torch.randint(0, diffusion_config['num_timesteps'], (latent_sample.shape[0],)).to(rank)
        ...
        logging.info(f'Epoch [{epoch_idx + 1}/{num_epochs}], Learning Rate: {current_lr:.4e} Loss: {epoch_loss:.4f} Noise Pred MEAN: {noise_pred.mean().item():.2e}')

...
from dataset.LucieLoader import LucieLoader
...
def sample(model, scheduler, train_config, diffusion_model_config,
                diffusion_config, dataset_config, SR_model, cond_data_index, dataset, lucie_dataset):
    ...
    orography = lres[:, 5:6, :, :]          # shape: [1, 1, H, W]
    land_sea_mask = lres[:, 6:7, :, :]      # shape: [1, 1, H, W]
    ...
    xt = torch.randn_like(hres).to(device)
    
    save_count = 0
    T = torch.full((1,), diffusion_config['num_timesteps'] - 1, dtype=torch.long, device=device)

    x_0 = model(xt, T.to(device), cond=cond_input) 
...
def infer(args):
    ...
    checkpoint_path = '/glade/derecho/scratch/mdarman/lucie/results/lazy_diff_v2/checkpoints/best_ldm.pth'
    ...
    lucie_dataset = LucieLoader(
        ...
        input_vars=['Temperature_7', 'Specific_Humidity_7', 'U-wind_3', 'V-wind_3', 'logp', 'tp6hr'],
            # input_vars = ['Temperature_7','Specific_Humidity_7', 'U-wind_3', 'V-wind_3', 'tp6hr', 'orography','land_sea_mask', 'logp', ],
        normalize=True
        )
    ...
    with torch.no_grad():
        ...
        for cond_data_index in range(args.start_index, len(dataset)):

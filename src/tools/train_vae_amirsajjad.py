...
from models.vae_amirsajjad import VAE
...
def train(args):
    ...
    for epoch_idx in range(num_epochs):
        ...
        circular_padding = torch.nn.CircularPad2d((80, 80, 79, 80))
        ...
        for batch_idx, data in enumerate(tqdm(data_loader)):
            ...
            lres_padded = circular_padding(lres_upsampled)
            ...
            # Adjusting output ' oro_padded[0, 0, 79:-80, 80:-80]
            # output = output[:,:,3:3+721,:] # (728, 1440) --Conv_layer--> (721, 1440)
            output[:, 0] += lres_padded[:, 0] 
            output[:, -1] += lres_padded[:, -3]
            ...
            recon_loss = spectral_sqr_abs2(output, hres_padded, lambda_fft=train_config['lambda_fft']) # recon_loss = recon_criterion(output, hres) 

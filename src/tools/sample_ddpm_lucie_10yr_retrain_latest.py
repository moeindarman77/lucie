...
def sample(model, scheduler, train_config, diffusion_model_config,
                ...
                orography_lr, land_sea_mask, orography_hr):
    ...
    # Use FNO output as conditioning (concatenate with high-res orography)
    ...
    for i in tqdm(reversed(range(diffusion_config['num_timesteps']))):
        ...
        if i == 0:
            ...
            output_dir = os.path.join(task_dir, 'samples_lucie_10yr_retrain_latest')
...
def infer(args):
    ...
    checkpoint_path = '/glade/derecho/scratch/mdarman/lucie/results/unet_final_v10_retrain/checkpoints/ddpm_ckpt.pth'
    if os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path)
        print(f'Loaded LATEST retrained diffusion model checkpoint from epoch {checkpoint["epoch"]}')
        new_state_dict = {}
        ...
        raise Exception('Diffusion model checkpoint not found')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Arguments for LUCIE 10yr sampling')
    parser.add_argument('--config', dest='config_path',
                        default='config/ERA5_config_lucie_10yr.yaml', type=str)

...
def sample(model, scheduler, train_config, diffusion_model_config,
    ...
    for i in tqdm(reversed(range(diffusion_config['num_timesteps']))):
        ...
        if i == 0:
            ...
            output_dir = os.path.join(task_dir, 'samples_lucie_10yr_retrain_best')
...
def infer(args):
    ...
    checkpoint_path = '/glade/derecho/scratch/mdarman/lucie/results/unet_final_v10_retrain/checkpoints/best_ldm.pth'
    if os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path)
        print(f'Loaded BEST retrained diffusion model checkpoint from epoch {checkpoint["epoch"]}')
        new_state_dict = {}

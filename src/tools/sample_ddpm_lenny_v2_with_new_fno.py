...
def sample(model, scheduler, train_config, diffusion_model_config,
    ...
    oro_expanded = oro_expanded.float().to(device)
    lres_upsampled[:, 5] = oro_expanded[:, 0]  # Replace the oro channel with the oro_expanded tensor
    ...
    # fno_output_padded = circular_padding(fno_output)
    ...
    # T = 500
    # xt = scheduler.add_noise(fno_output, xt, T)
    # xt = fno_output.to(device)
...
def infer(args):
    ...
    checkpoint_path = '/glade/derecho/scratch/mdarman/lucie/results/unet_simple_v_with_new_fno/checkpoints/best_ldm.pth'

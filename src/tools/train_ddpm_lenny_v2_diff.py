...
def train(rank, world_size, args):
    ...
    if dataset_config['land_sea_mask'] == 1:
        ...
        # oro = torch.tensor(np.load("/glade/derecho/scratch/mdarman/lucie/oro.npz")['geopotential_normalized']).unsqueeze(0).unsqueeze(1)
    else:
        lsm = None
    if dataset_config['orography'] == 1:
        print("Using orography")
        oro = torch.tensor(np.load(dataset_config['orography_dir'])['geopotential_normalized']).unsqueeze(0).unsqueeze(1)
    else:
        oro = None
    ...
    checkpoint = torch.load(SR_model_load_dir, map_location=lambda storage, loc: storage.cuda(rank))
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_idx, data in enumerate(tqdm(data_loader, disable=(rank != 0))):
            ...
            # Expand oro to match batch size B
            oro_expanded = oro.expand(fno_output.shape[0], -1, -1, -1)
            oro_expanded = oro_expanded.float().to(rank)

            # Concatenate along the channel dimension (C)
            cond_input = torch.cat([fno_output, oro_expanded], dim=1)  # New shape: (B, C+1, W, H)

            # Save cond input into the directory
            # if batch_idx == 0:
            #     np.save(os.path.join("/glade/derecho/scratch/mdarman/lucie", "hres_with_oro.npy"), cond_input.cpu().detach().numpy())
            #     # Save lres_upsampled
            #     np.save(os.path.join("/glade/derecho/scratch/mdarman/lucie", "lres_upsampled.npy"), lres_upsampled.cpu().detach().numpy())
            #     print(f"File saved: ")
                        
            # Set conditional to be the high resolution input
            # cond_input = hres
            # hres_padded = circular_padding(hres)
            ...
            noisy_im = scheduler.add_noise(hres-fno_output, noise, t)

...
def train(rank, world_size, args):
    ...
    # Priority: 1) Latest checkpoint in new task dir, 2) Start from scratch
    # NOTE: Since jobs have already run, we start from the latest checkpoint in the task directory
    ...
    # backup_checkpoint = '/glade/derecho/scratch/mdarman/lucie/results/unet_final_v10/checkpoints/best_ldm_BACKUP_20251015.pth'
    ...
    # # Fall back to backup checkpoint for first job
    # elif os.path.exists(backup_checkpoint):
    #     resume_checkpoint = backup_checkpoint
    #     logging.info(f"Using backup checkpoint for initial training: {backup_checkpoint}")
    else:
        logging.info(f"No checkpoint found at {latest_checkpoint}, starting from scratch.")
    ...
    for epoch_idx in range(start_epoch, num_epochs):
        ...
        for batch_idx, data in enumerate(tqdm(data_loader, disable=(rank != 0))):
            ...
            cond_input = torch.cat([fno_output, hres[:, 5:6]], dim=1) # Conctaenate the output of the FNO model with the orography as conditioning

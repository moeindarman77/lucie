...
def sample(model, scheduler, train_config, diffusion_model_config,
    ...
    cond_input = lres_upsampled.to(device)
    ...
    for i in tqdm(reversed(range(diffusion_config['num_timesteps']))):
        ...
        model.to(device)
        noise_pred = model(xt, torch.as_tensor(i).unsqueeze(0).to(device), cond_input=cond_input) 
...
def infer(args):
    ...
    model.eval()
    checkpoint_path = '/glade/derecho/scratch/mdarman/lucie/results/fno_v2/checkpoints/ddpm_ckpt_epoch_20.pth'

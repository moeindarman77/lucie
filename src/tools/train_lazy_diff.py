...
def train(rank, world_size, args):
    ...
    for epoch_idx in range(start_epoch, num_epochs):
        ...
        for batch_idx, data in enumerate(tqdm(data_loader, disable=(rank != 0))):
            ...
            # Forward pass
            predicted_output = model(noisy_im, t, cond=cond_input)

            loss = spectral_sqr_abs2(predicted_output, hres, lambda_fft=train_config['lambda_fft'])
            # loss = criterion(noise_pred, noise)
            losses.append(loss.item())
            loss.backward()
            # Trying with gradient clipping
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) # Previously 1.0
            optimizer.step()

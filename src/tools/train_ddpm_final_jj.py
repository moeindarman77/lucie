...
def train(rank, world_size, args):
    ...
    resume_checkpoint = '/glade/derecho/scratch/mdarman/lucie/results/unet_final_v4/checkpoints/ddpm_ckpt_s1.pth'
    ...
    for epoch_idx in range(start_epoch, num_epochs):
        ...
        for batch_idx, data in enumerate(tqdm(data_loader, disable=(rank != 0))):
            ...
            output = torch.cat([hres[:,0:1], hres[:,4:5]], dim=1)
            ...
            ##########################
            # Sample random noise
            noise = torch.randn_like(output).to(rank)
            
            # Sample timestep
            t = torch.randint(0, diffusion_config['num_timesteps'], (output.shape[0],)).to(rank)
            
            # Add noise to images according to timestep
            noisy_im = scheduler.add_noise(output, noise, t)

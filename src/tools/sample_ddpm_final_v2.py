...
def sample(model, scheduler, train_config, diffusion_model_config,
    ...
    cond_input = torch.cat([fno_output_lucie, hres[:, 5:6]], dim=1) # Conctaenate the output of the FNO model with the orography as conditioning
    ...
    # for i in tqdm(reversed(range(T))):
        ...
        # print(f"Noise prediction MEAN: {noise_pred.mean(axis=(0,2,3))}")
        
        # Use scheduler to get x0 and xt-1
        xt, x0_pred = scheduler.sample_prev_timestep(xt, noise_pred, torch.as_tensor(i).to(device))
        # print(xt.mean(axis=(0,2,3)))
...
def infer(args):
    ...
    checkpoint_path = '/glade/derecho/scratch/mdarman/lucie/results/unet_final_v10/checkpoints/best_ldm.pth'
